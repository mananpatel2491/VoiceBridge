...
def update_docs(requested_model=None, dry_run=False):
    ...
    prompt = """
    Explain how to use Gemini Code Assist for the VoiceBridge project.
    Specifically cover:
    ...
    3. How to use the VoiceBridge Maintenance Skills (verify_structure.py, optimize_changelog.py, update_getting_started.py).
    ...
    5. VoiceBridge-specific guidance: how to use Agent mode to iterate on the voice pipeline (STT / LLM / TTS stages) while maintaining the provider interface pattern.
    ...
    try:
        ...
        output_content = (
            f"# Getting Started with Gemini Code Assist — VoiceBridge (Auto-Updated)\n\n"
            f"{content}\n\n"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Update VoiceBridge onboarding docs via Gemini API.")
