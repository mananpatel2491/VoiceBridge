...
A production-grade voice AI pipeline framework designed to bridge voice input/output with AI backends through a multi-agent autonomous development workflow. Transforms high-level English intent into scalable voice software using a multi-LLM stack (Claude, Gemini, and local Ollama) while maintaining strict architectural integrity through a Project Constitution.

## Core Pipeline

```
Audio Capture → STT → LLM Processing → TTS → Audio Output
```

Each stage is modular and swappable via a provider interface. See `PATTERNS.md` for the Voice Pipeline Patterns.
...
*Refer to `GEMINI.md` for the Project Constitution and non-negotiable operating procedures.*
